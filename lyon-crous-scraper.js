...
const CAMPUS_REFERENCE_COORDS = {
  lat: 45.7827,
  lng: 4.8771,
  name: 'Campus La Doua - Université Lyon 1',
...
const RESIDENCE_COORDS = {
  ...
  'André Lirondelle': { lat: 45.7620, lng: 4.8580 },
...
async function createGitHubIssue(housingList) {
  if (!GITHUB_TOKEN || !GITHUB_REPOSITORY) {
    console.log('⚠️  GitHub not configured, logging to console only');
    return;
  }
  ...
  const priorityLabel = hasPriority ? '⭐ PRIORITY' : '';
  
  // Build issue body
  let body = `## 🏠 ${housingList.length} New Housing Found!\n\n`;
  body += `**Time:** ${now}\n`;
  body += `**Reference:** ${CAMPUS_REFERENCE_COORDS.name}\n\n`;
  body += `---\n\n`;
  
  sorted.forEach((h, i) => {
    const priority = h.isPriority ? '⭐ **PRIORITY**' : '';
    body += `### ${i + 1}. ${h.name} ${priority}\n\n`;
    body += `| Info | Value |\n|------|-------|\n`;
    body += `| 🔗 Link | [Apply Now](${h.link}) |\n`;
    body += `| 📏 Distance | ${h.distance.toFixed(2)} km |\n`;
    body += `| 🚶 Walking | ${h.timeEstimate.walking} min |\n`;
    body += `| 🚴 Biking | ${h.timeEstimate.biking} min |\n`;
    body += `| 🚇 Metro | ${h.timeEstimate.metro} min |\n\n`;
  });
  ...
  const title = `🏠 ${housingList.length} New CROUS Housing ${priorityLabel} - ${new Date().toLocaleDateString('fr-FR')}`;
  ...
  try {
    ...
    if (frCardExists) {
      ...
      $('.fr-card').each((index, element) => {
        ...
        console.log(`📍 ${name} | ${distance.toFixed(1)}km | ${isPriority ? '⭐' : '📍'}`);
...
async function main() {
  console.log('🚀 CROUS Lyon Scraper');
  console.log(`📍 Reference: ${CAMPUS_REFERENCE_COORDS.name}`);
  console.log(`🔄 Mode: ${CI_MODE ? 'GitHub Actions' : 'Local'}`);
